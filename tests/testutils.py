...
def packet_to_match(packet):
    """
    Convert a packet to match structure
    """
    idx = 0
    try:
        idx = packet._parse_l2(idx)
            
        if self.match.oxm_fields.index([(((((ofp.OFPXMC_OPENFLOW_BASIC << 7) + ofp.OFPXMT_OFB_ETH_TYPE) << 1) +
                      0) << 8 )+ 2,ETHERTYPE_IP]) != ValueError:
